...
Outputs land in `demo_photos/` (GIF/PNG per slide, plus an `_preview.png`
of each final frame for quick review) and the assembled deck at
`W02_How_Text_Becomes_Numbers.pptx`. Neither is committed — regenerate at
will; the scripts are the source of truth.

GIFs animate in PowerPoint's slideshow mode (not in the editor). Each plays
once per slide visit and holds its final frame.
